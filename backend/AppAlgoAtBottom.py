...
# Attributes that are most applicable to our use case seem to be Temp, Energy, and Valence (then maybe danceability) that 
### Categorized into 

# goes from Heatwave all the way down to Snowstorm
# Listed in order from corresponding highest temperature to lowest temperature song requirements.

def Heatwave(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp >= 294.261 and description_weather("Relating to Heat or Heatwave from Weatherify"): # Fahrenheit 80 Degrees
        return song_id[valence > 0.55 and danceability < 0.6 and energy < 0.6] * 20
                            # Upper-Middle/Half        # Upper Half       # Upper Half

def Sunshine(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp >= 288.706 and description_weather("Relating to Sun or Sunshine from Weatherify"): # Fahrenheit 80 Degrees
        return song_id[valence > 0.55 and danceability < 0.5 and energy < 0.6] * 20
                            # Upper-Middle/Half        # General Middle     # Upper Half

def Cloudy(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]

    if temp >= 283.15 and description_weather("Relating to Clouds from Weatherify"): # Fahrenheit 50 Degrees
        return [(valence > 0.4 and valence < 0.6) and danceability >= 0.4 and (0.4 > energy and energy < 0.6)]
                        # Middle of the Pack                # Middle of the Pack            # Middle of the Pack


def Chilly(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp >= 283.15 and description_weather("Relating to Chilly"): # Fahrenheit 50 Degrees
        return [(valence > 0.3 and valence < 0.6) and danceability >= 0.35 and (0.35 > energy and energy < 0.7)]
                        # Lower-Middle Half of the Pack          # Lower-Middle of the Pack        # Middle of the Pack, Range Expanded


def Storming(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp <= 280.372 and description_weather("Relating to Storm from Weatherify"): # Farienheit 45 Degrees
        return song_id[(valence > 0.3 and valence < 0.4) and (danceability > 0.2 and danceability < 0.8) and (0.5 > energy and energy < 0.2)] * 20
                            # Lower End                     # Enormous Range Between General Middle                 # Lower End 


def Raining(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp >= 277.594 and description_weather("Relating to Rain from Weatherify"): # Fahrenheit 40 Degrees
        return song_id[(valence > 0.4 and valence < 0.6) and  (danceability >0.3 and danceability < 0.7) and (energy > 0.4 and energy < 0.6) ] * 20
                            # General Middle Range                # Larger Range Between General Middle             # Between General Middle


def Frozen(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp <= 274.817 and description_weather("Relating to Snow or Ice from Weatherify"): # Farienheit 35 Degrees
        return song_id[(valence > 0.3 and valence < 0.4) and danceability < 0.5 and (0.5 > energy and energy < 0.2)] * 20
                            # Lower End                     # Tranquil, not Crazy          # Lower End 


def Blizzard(song_id, temp, description_weather):
    valence = song_id["valence"]
    danceability = song_id["danceability"]
    energy = song_id["energy"]
    if temp <= 272.039 and description_weather("Relating to Snow or Snowstorm or Blizzard from Weatherify"): # Farienheit 30 Degrees
        return song_id[(valence > 0.3 and valence < 0.4) and (danceability > 0.3 and danceability < 0.6) and (0.5 > energy and energy < 0.3)] * 20
                            # Lower End                     # General Middleish                                 # General Lower-Middle Half
